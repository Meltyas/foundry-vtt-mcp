...
export class FoundryDataAccess {
   ...
   */
  private async getOrCreateNestedItemFolder(folderPath: string): Promise<string | null> {
    ...
    for (const part of parts) {
      const existing = game.folders?.find(
        (f: any) => f.name === part && f.type === 'Item' && (f.folder?.id ?? null) === parentId
      );
  ...
  async createWorldItems(params: {
    ...
    for (const it of items) {
      ...
      try {
        const itemData: Record<string, any> = { name: it.name, type: it.type };
        if (it.img) itemData.img = it.img;
        if (it.system && typeof it.system === 'object') itemData.system = it.system;
        if (folderId) itemData.folder = folderId;

        const doc = await Item.create(itemData as any);
        if (doc) {
  ...
  async listWorldItems(params: {
    ...
    folder?: string;
    nameFilter?: string;
  }): Promise<Array<{ id: string; name: string; type: string; folder: string | null }>> {
    this.validateFoundryState();

    let items: any[] = Array.from(game.items ?? []);
    ...
    if (params.nameFilter) {
      ...
      items = items.filter((i: any) => i.name?.toLowerCase().includes(lower));
    }

    if (params.folder) {
      const lower = params.folder.toLowerCase();
      items = items.filter((i: any) => {
        const folderName: string = i.folder?.name ?? '';
