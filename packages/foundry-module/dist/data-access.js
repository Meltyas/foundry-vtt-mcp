...
class PersistentCreatureIndex {
    ...
    extractDnD5eCreatureData(doc, pack) {
        try {
            ...
            let size = system.traits?.size?.value ||
                system.traits?.size ||
                system.size?.value ||
                system.size ||
                system.details?.size ||
            ...
            const hitPoints = system.attributes?.hp?.max ||
                system.hp?.max ||
            ...
            const armorClass = system.attributes?.ac?.value ||
                system.ac?.value ||
            ...
            let alignment = system.details?.alignment?.value ||
    ...
    extractPF2eCreatureData(doc, pack) {
        try {
            ...
            const creatureType = traits.find((t) => creatureTraits.includes(t.toLowerCase()))?.toLowerCase() ||
                'unknown';
     ...
     */
    async buildCosmereRpgIndex(_force = false) {
        this.buildInProgress = true;
        const startTime = Date.now();
        let progressNotification = null;
        let totalErrors = 0;
     ...
     */
    async extractCosmereRpgDataFromPack(pack) {
        const creatures = [];
        let errors = 0;
        try {
            const documents = await pack.getDocuments();
            for (const doc of documents) {
                try {
                    ...
                    console.warn(`[${this.moduleId}] Failed to extract Cosmere RPG data from ${doc.name} in ${pack.metadata.label}:`, error);
                    errors++;
                }
            }
        }
     ...
     */
    extractCosmereRpgCreatureData(doc, pack) {
        try {
            const system = doc.system ?? {};
            const tier = typeof system.tier === 'number' ? system.tier : 0;
            const role = typeof system.role === 'string' && system.role.length > 0
                ? system.role.toLowerCase()
                : 'unknown';
            const size = typeof system.size === 'string' && system.size.length > 0
                ? system.size.toLowerCase()
                : 'medium';
            const creatureType = typeof system.type?.id === 'string' && system.type.id.length > 0
                ? system.type.id.toLowerCase()
                : 'unknown';
            const subtype = typeof system.type?.subtype === 'string' && system.type.subtype.length > 0
                ? system.type.subtype
                : '';
...
export class FoundryDataAccess {
    ...
    extractSpellcastingData(actor) {
        ...
        if (systemId === 'pf2e') {
            ...
            const spellcastingEntries = actorAny.spellcasting?.contents ||
                actorAny.items?.filter((i) => i.type === 'spellcastingEntry') ||
                [];
            ...
            const focusSpells = spellItems.filter((spell) => {
                const spellSystem = spell.system;
                return (spellSystem?.traits?.value?.includes('focus') || spellSystem?.category?.value === 'focus');
            });
            ...
            for (const classItem of classes) {
                ...
                if (classSystem?.spellcasting?.progression &&
    ...
    passesFilters(entry, filters) {
        ...
        if (filters.hasLegendaryActions !== undefined) {
            const hasLegendary = !!(system.resources?.legact ||
                system.legendary ||
                (system.resources?.legres && system.resources.legres.value > 0));
        ...
        if (filters.spellcaster !== undefined) {
            const isSpellcaster = !!(system.spells ||
                system.attributes?.spellcasting ||
                (system.details?.spellLevel && system.details.spellLevel > 0));
     ...
     */
    passesCosmereRpgCriteria(creature, criteria) {
        if (criteria.tier !== undefined) {
            if (typeof criteria.tier === 'number') {
                if (creature.tier !== criteria.tier)
                    return false;
            }
            else {
                const { min, max } = criteria.tier;
                if (min !== undefined && creature.tier < min)
                    return false;
                if (max !== undefined && creature.tier > max)
                    return false;
            }
        }
        ...
        if (criteria.hasInvestiture !== undefined &&
        ...
        if (hpRange !== undefined) {
            if (typeof hpRange === 'number') {
                if (creature.hitPoints !== hpRange)
                    return false;
            }
            else {
                const { min, max } = hpRange;
                if (min !== undefined && creature.hitPoints < min)
                    return false;
                if (max !== undefined && creature.hitPoints > max)
                    return false;
            }
        }
        if (criteria.defensesMin) {
            const { phy, cog, spi } = criteria.defensesMin;
            if (phy !== undefined && creature.defensePhysical < phy)
                return false;
            if (cog !== undefined && creature.defenseCognitive < cog)
                return false;
            if (spi !== undefined && creature.defenseSpiritual < spi)
                return false;
        }
    ...
    passesCriteria(entry, criteria) {
        ...
        if (criteria.creatureType) {
            ...
            const entryType = system.details?.type?.value ||
                system.details?.type ||
                system.type?.value ||
                system.type ||
                '';
    ...
    removeSensitiveFields(obj, visited = new WeakSet(), depth = 0) {
        ...
        try {
            ...
            const isDnd5eSensesShape = keys.includes('ranges') && keys.some(k => DEPRECATED_DND5E_SENSE_KEYS.includes(k));
    ...
    resolveTargetPlayer(targetPlayer) {
        ...
        const anyCharacter = game.actors?.find((actor) => {
            if (!actor.name)
                return false;
            return (actor.name.toLowerCase() === searchTerm || actor.name.toLowerCase().includes(searchTerm));
        });
        ...
        return {
            ...
            errorMessage: `No player or character named "${targetPlayer}" found. Available players: ${allUsers
                .filter(u => !u.isGM)
                .map(u => u.name)
                .join(', ') || 'none'}`,
        };
    ...
    buildRollFormula(rollType, rollTarget, rollModifier, character) {
        ...
        if (rollModifier && rollModifier.trim()) {
            const modifier = rollModifier.startsWith('+') || rollModifier.startsWith('-')
                ? rollModifier
                : `+${rollModifier}`;
    ...
    async setActorOwnership(data) {
        ...
        try {
            ...
            const permissionName = permissionNames[data.permission] ||
    ...
    /**
     * Get or create a nested folder path for Items (e.g. "Parent/Child").
     * Returns the ID of the deepest folder in the path.
     */
    async getOrCreateNestedItemFolder(folderPath) {
        const parts = folderPath.split('/').map(p => p.trim()).filter(Boolean);
        if (parts.length === 0)
            return null;
        let parentId = null;
        for (const part of parts) {
            const existing = game.folders?.find((f) => f.name === part && f.type === 'Item' && (f.folder?.id ?? null) === parentId);
            if (existing) {
            ...
            try {
                ...
                console.warn(`[${this.moduleId}] Failed to create folder "${part}":`, error);
                return parentId; // Return deepest successfully created folder
            }
        }
        return parentId;
    }
        ...
        for (const patch of updates) {
            ...
            try {
                ...
                if (patch.system !== undefined) {
                    const currentSystem = item.system?.toObject
    ...
    async createWorldItems(params) {
        ...
        if (folder) {
            folderId = folder.includes('/')
                ? await this.getOrCreateNestedItemFolder(folder)
                : await this.getOrCreateFolder(folder, 'Item');
        }
        const created = [];
        const errors = [];
        for (const it of items) {
            if (!it.name || !it.type) {
                errors.push(`Skipped item with missing name or type: ${JSON.stringify(it)}`);
                continue;
            }
            try {
                const itemData = { name: it.name, type: it.type };
                if (it.img)
                    itemData.img = it.img;
                if (it.system && typeof it.system === 'object')
                    itemData.system = it.system;
                if (folderId)
                    itemData.folder = folderId;
                const doc = await Item.create(itemData);
                if (doc) {
                    created.push({ id: doc.id, name: doc.name, type: doc.type });
                }
            }
            catch (error) {
                const msg = `Failed to create item "${it.name}": ${error instanceof Error ? error.message : 'Unknown error'}`;
                errors.push(msg);
                console.error(`[${this.moduleId}] ${msg}`, error);
            }
        }
        this.auditLog('createWorldItems', { count: items.length, folder }, 'success');
        return { created, errors };
    }
    /**
     * List world-level Item documents with optional filters.
     */
    async listWorldItems(params) {
        this.validateFoundryState();
        let items = Array.from(game.items ?? []);
        if (params.type) {
            items = items.filter((i) => i.type === params.type);
        }
        if (params.nameFilter) {
            const lower = params.nameFilter.toLowerCase();
            items = items.filter((i) => i.name?.toLowerCase().includes(lower));
        }
        if (params.folder) {
            const lower = params.folder.toLowerCase();
            items = items.filter((i) => {
                const folderName = i.folder?.name ?? '';
                return folderName.toLowerCase().includes(lower);
            });
        }
        return items.map((i) => ({
            id: i.id,
    ...
    async toggleTokenCondition(data) {
        ...
        try {
            ...
            const condition = conditions.find((c) => c.id === data.conditionId || c.name?.toLowerCase() === data.conditionId.toLowerCase());
            if (!condition) {
    ...
    async useItem(params) {
        ...
        const item = actor.items.find((i) => i.id === itemIdentifier || i.name.toLowerCase() === itemIdentifier.toLowerCase());
        if (!item) {
        ...
        try {
            ...
            const targetInfo = resolvedTargetNames.length > 0 ? ` targeting ${resolvedTargetNames.join(', ')}` : '';
            const result = {
                success: true,
                status: 'initiated',
