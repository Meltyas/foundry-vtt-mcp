...
export class CompendiumTools {
        ...
        if (item.type === 'npc' || item.type === 'character' || item.type === 'adversary') {
            ...
            if (gameSystem === 'cosmere-rpg') {
                ...
                if (typeof system.tier === 'number')
                    stats.tier = system.tier;
                if (typeof system.level === 'number')
                    stats.level = system.level;
                if (typeof system.role === 'string' && system.role)
                    stats.role = system.role.toLowerCase();
                ...
                if (typeof system.size === 'string' && system.size)
                    stats.size = system.size.toLowerCase();
    ...
    formatCreatureListItem(creature, gameSystem) {
        ...
        if (gameSystem === 'cosmere-rpg') {
            ...
            if (creature.tier !== undefined)
                formatted.tier = creature.tier;
            if (creature.role)
                formatted.role = creature.role;
            if (creature.subtype)
                formatted.subtype = creature.subtype;
            if (creature.creatureType)
                formatted.creatureType = creature.creatureType;
            if (creature.size)
                formatted.size = creature.size;
            if (creature.hitPoints !== undefined)
                formatted.hitPoints = creature.hitPoints;
            if (creature.focus !== undefined)
                formatted.focus = creature.focus;
            if (creature.investiture !== undefined)
                formatted.investiture = creature.investiture;
            if (creature.hasInvestiture !== undefined)
                formatted.hasInvestiture = creature.hasInvestiture;
            if (creature.defenses)
                formatted.defenses = creature.defenses;
            if (creature.deflect !== undefined)
                formatted.deflect = creature.deflect;
            if (creature.walkSpeed !== undefined)
                formatted.walkSpeed = creature.walkSpeed;
            if (creature.summary)
                formatted.summary = creature.summary;
            return formatted;
        }
        if (gameSystem) {
            ...
            const hasSpells = creature.hasSpells ??
                !!(system.spells ||
                    system.attributes?.spellcasting ||
                    (system.details?.spellLevel && system.details.spellLevel > 0));
            const hasLegendary = creature.hasLegendaryActions ??
    ...
    describeCriteria(params, gameSystem) {
        ...
        if (gameSystem === 'dnd5e') {
            ...
            if (params.role)
                parts.push(`role=${String(params.role).toLowerCase()}`);
            ...
            if (params.deflectMin !== undefined)
                parts.push(`deflect>=${params.deflectMin}`);
        }
