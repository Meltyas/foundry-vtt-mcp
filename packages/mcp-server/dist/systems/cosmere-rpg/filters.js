 ...
 */
export const COSMERE_ROLES = ['minion', 'rival', 'boss'];
/**
...
export const COSMERE_SIZES = ['tiny', 'small', 'medium', 'large', 'huge', 'gargantuan'];
...
export function matchesCosmereRpgFilters(creature, filters) {
    ...
    if (filters.defensesMin !== undefined) {
        ...
        if (phy !== undefined && (def.phy ?? -Infinity) < phy)
            return false;
        if (cog !== undefined && (def.cog ?? -Infinity) < cog)
            return false;
        if (spi !== undefined && (def.spi ?? -Infinity) < spi)
            return false;
    }
    if (filters.deflectMin !== undefined) {
        if ((data.deflect ?? -Infinity) < filters.deflectMin)
            return false;
    }
    return true;
}
export function describeCosmereRpgFilters(filters) {
    const parts = [];
    if (filters.tier !== undefined) {
        parts.push(typeof filters.tier === 'number'
            ? `tier ${filters.tier}`
            : describeRange('tier', filters.tier));
    }
    if (filters.role)
        parts.push(`role=${filters.role.toLowerCase()}`);
    if (filters.creatureType)
        parts.push(`type=${filters.creatureType.toLowerCase()}`);
    if (filters.size)
    ...
    if (filters.health !== undefined) {
        parts.push(typeof filters.health === 'number'
            ? `hp=${filters.health}`
    ...
    if (filters.deflectMin !== undefined)
        parts.push(`deflect>=${filters.deflectMin}`);
