...
export class DSA5CharacterCreator {
    ...
    async handleListArchetypes(args) {
        ...
        try {
            ...
            const characterPacks = packs.filter((pack) => pack.type === 'Actor' && pack.system === 'dsa5' && (!packId || pack.id === packId));
            const archetypes = [];
    ...
    formatArchetypeListResponse(archetypes, filterBySpecies, filterByProfession) {
        ...
        const archetypeList = archetypes
            .map(archetype => `• **${archetype.name}** (${archetype.species}, ${archetype.profession})\n  Pack: ${archetype.packLabel} | ID: ${archetype.id}`)
            .join('\n\n');
        return {
            summary,
